...
| `EnableConsoleLog` | `Diagnóstico en consola (interruptor maestro)` / `Console diagnostics (master switch)` | No | Líneas LOSS, WARN e INFO. El fichero persistente se controla por separado. |
| `RelayServerLogsToClients` | `Reenviar logs del dedicado a administradores` / `Relay dedicated-server logs to administrators` | No | Solo administradores suscritos, con revalidación por envío y limpieza al desconectarse o perder acceso. |
...
Con consola activada, `ClientVisual` deja una línea `[CLI]` al enviar el primer estado útil o uno más rico, y `ClientServerCompare` una línea `[SRV]` por cadáver auditado. Distingue `clientReport=received clientVisuals=0` de `clientReport=missing clientVisuals=0`. `VerboseDebug` se retira y sus valores heredados no reactivan DETAIL; no se alteran las capturas ni los ficheros persistentes.

Cada reproducción indica las opciones mínimas ES/EN y conserva `console.txt` más los ficheros persistentes configurados. Consola y relé están OFF por defecto; el servidor revalida al administrador en cada envío y elimina la suscripción al perder acceso o desconectarse.
